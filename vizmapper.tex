...
There are a number of terms that are used in the remainder of the chapter that are helpful to define. The specifics of the definitions are debatable and are not cited, however they remain helpful for understanding the rest of this chapter.
...
\item[Interpreter] the software program takes programmer code as an executable to run a program
\end{description}
...
\begin{figure}[htb]
\centering
\subfloat[First level of hierarchy][First level of hierarchy]{
\includegraphics[width=0.45\textwidth]{uncluttered_balloon.png}
\label{fig:unclutteredBalloon}}
\subfloat[One branch of the second level][One branch of the second level]{
\includegraphics[width=0.45\textwidth]{interactive_balloon.png}
...
The Vizmapper interface provides all the functionality necessary to create a mapping on a Mapper network. The hope is that by applying an understanding of the mapping task, user interface design principles, data visualization principles, and a basic understanding of human cognitive processes that Vizmapper is as effective of an interface for configuring mappings over a MSN with many signals as the original Dotmapper is for configuring mappings on a DMI with relatively fewer signals.
