...
There are any number of very stable development environments that enable a software developer to write a single corpus of computer code and create applications that can run on Windows, OSX, and Linux. This choices effects primarily the developer of the software and not the user of the software. The choice of deployment environment is much more consequential to the user because it effects how they will actually run the software.

There are three common methods for a user to run a piece of software on their chosen machine.

The user can install a pre-compiled binary image of the piece of software, which means that the developer has written code and produced a file that can be executed without any translation on the user's specific machine/operating system combination because the file is machine code that is run directly on the machine. If there is no available pre-compiled binary for a specific machine architecture, the user can obtain the source code of the software and compile a binary file that works on their specific machine as long as a compiler exists for their machine.

The first method is an option when the software is developed in a \emph{compiled language} environment. There are also environments that use \emph{interpreted languages} or \emph{script languages}. An interpreted language differs from a compiled language because unlike a compiled language that uses the process of feeding the program code into a compiler and producing a binary that can be executed to run the program, an interpreted language uses the process of interpreting the program code to directly run the program. The interpretor is responsible for interfacing with the lower level machine for the higher level interpreted language when the program code is run. A compiled program interfaces directly with the lower level machine because it has already been converted into lower level machine code. A web application with internal logic is essentially an interpreted program because a web browser that runs HTML, CSS, and Javascript downloads the program code and uses an internal interpretor to execute the program. It does not download a compiled program. Distributing software as a script to be run in an interpretor is attractive because the burden for implementing the interpretor for various different machine architecture is moved to the developer of the actual language and the developer can count on a user being able to run their program as long as the interpretor for the language is available for the user's machine.
