...
There is more to be said about the concept of a one-to-one mapping. The framework for discussing different types of mappings is provided in one form by the mathematical concept of \emph{cardinality}.
...
A relational database can be seen as a very robust and flexible data labeling system in reference to the discussion in section \ref{sec:Mappings in Computer Science}. A relational database is defined as a collection of relations, where a relation is defined as a set of lists of attributes. Each list in the set has the same attributes as every other list in the set. An attribute defines a property that can take any value constrained by the values that make sense for each particular attribute. A relation is usually described by a table as in Table \ref{tab:relation}.

\begin{table}
    \begin{center}
    \begin{tabular}{ | l | l | l | }
    \hline
    Student ID : Integer  & Thesis Topic : String & Thesis Advisor : String \\ \hline
    260311100 & A Tool for Configuring Mappings & Marcelo Wanderley \\ \hline
    131033222 & An Invisible, Anti-Gravity Toaster & Ohno Hedidnt \\
    \hline
    \end{tabular}
    \end{center}
    \caption{Example of a database relation}
    \label{tab:relation}
\end{table}
